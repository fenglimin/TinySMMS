/*************************************************
  ...
  Author 	: Forest Yue
  Created	: Aug. 1, 2002
  Comments 	: Ado Class .cpp file. Ado module is a class for operating database with ado. 
*************************************************/
#include "stdafx.h"
#include "ado.h"
#include <math.h>
...
void CADODatabase::RollbackTransaction()
{
	try
	{
		if(IsOpen())
...
void CADODatabase::CommitTransaction()
{
	try
	{
		if(IsOpen())
...
void CADODatabase::BeginTransaction()
{
	try
	{
		if(IsOpen())
...
void CADODatabase::SetLastErrorCode()
{
	m_dwLastError=0;
}
...
{	
	...
	//	m_pRecordset->CursorLocation = adUseClient;
		m_pRecordset->CursorLocation = adUseServer;//modified 25/11/2004
...
{	
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			/////////////////////////////////////////////
			//modified by thomas jiang, 18/03/2005
			//val = vtFld.decVal.Lo32;
			val = (signed __int64)vtFld.decVal.Lo64 ;
			/////////////////////////////////////////////
...
BOOL CADORecordset::GetFieldValue(LPCTSTR lpFieldName, CString& strValue, CString strDateFormat)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			double val = (signed __int64)vtFld.decVal.Lo64 ;
			/////////////////////////////////////////////
			val *= (vtFld.decVal.sign == 128)? -1 : 1;
			val /= pow(10.0, vtFld.decVal.scale); 
			str = DblToStr(val);
...
BOOL CADORecordset::GetFieldValue(int nIndex, CString& strValue, CString strDateFormat)
{
	...
	try
	{
		...
		{
		case VT_BOOL:
			str = (vtFld.boolVal? _T("true"): _T("false"));
			break;
		case VT_R4:
			str = DblToStr(vtFld.fltVal);
			break;
...
{
	FieldPtr pField = m_pRecordset->Fields->GetItem(lpFieldName);
	BOOL bTemp=GetChunk(pField, strValue);	
	return bTemp;
}
...
BOOL CADORecordset::Execute(CADOCommand* pAdoCommand)
{
	...
	if(pAdoCommand->GetText().IsEmpty())
		return FALSE;
	try
	{
...
{
	if(pAdoParameter->GetParameter() == NULL)
		return FALSE;
...
BOOL CADOParameter::GetValue(double& dbValue)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			dblVal = (signed __int64)vtVal.decVal.Lo64 ;
			/////////////////////////////////////////////
...
BOOL CADOParameter::GetValue(CString& strValue, CString strDateFormat)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			/////////////////////////////////////////////
			//modified by thomas jiang, 18/03/2005
			//double dblVal = vtVal.decVal.Lo32;
			double dblVal = (signed __int64)vtVal.decVal.Lo64 ;
			/////////////////////////////////////////////
			dblVal *= (vtVal.decVal.sign == 128)? -1 : 1;
			dblVal /= pow(10.0, vtVal.decVal.scale); 
			strVal = DblToStr(dblVal);
			}
			break;
		case VT_DATE:
			{
...
void CADORecordset::SetLastErrorCode()
{
	m_dwLastError=0;
}
